<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Avaliação de Desempenho') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('Performance-Reviews.update', $PerformanceReview->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="id_employee" 
                                ...
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}"
                                        {{ $employee->id == old('id_employee', $PerformanceReview->id_employee) ? 'selected' : '' }}>
                                        {{ $employee->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="nota" 
                                   id="nota" 
                                   value="{{ old('nota', $PerformanceReview->nota) }}"
                                   min="0"
                                   max="10"
                                   step="0.1"
                        ...
                        <div>
                            ...
                            <input type="date" 
                                   name="data_avaliacao" 
                                   id="data_avaliacao" 
                                   value="{{ old('data_avaliacao', $PerformanceReview->data_avaliacao) }}"
                        ...
                        <div>
                            ...
                            <textarea name="observacao" 
                                      ...
                                      placeholder="Descreva os pontos relevantes da avaliação...">{{ old('observacao', $PerformanceReview->observacao) }}</textarea>
                            @error('observacao')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
