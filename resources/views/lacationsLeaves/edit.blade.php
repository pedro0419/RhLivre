        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('lacations-leaves.update', $lacationsLeave->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="employee_id" 
                                ...
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}"
                                        {{ $employee->id == old('employee_id', $lacationsLeave->employee_id) ? 'selected' : '' }}>
                                        {{ $employee->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="tipo_ferias" 
                                    ...
                                    required>
                                <option value="Férias" {{ old('tipo_ferias', $lacationsLeave->tipo_ferias) == 'Férias' ? 'selected' : '' }}>Férias</option>
                                <option value="Licença Médica" {{ old('tipo_ferias', $lacationsLeave->tipo_ferias) == 'Licença Médica' ? 'selected' : '' }}>Licença Médica</option>
                                <option value="Licença Maternidade" {{ old('tipo_ferias', $lacationsLeave->tipo_ferias) == 'Licença Maternidade' ? 'selected' : '' }}>Licença Maternidade</option>
                                <option value="Licença Paternidade" {{ old('tipo_ferias', $lacationsLeave->tipo_ferias) == 'Licença Paternidade' ? 'selected' : '' }}>Licença Paternidade</option>
                                <option value="Outros" {{ old('tipo_ferias', $lacationsLeave->tipo_ferias) == 'Outros' ? 'selected' : '' }}>Outros</option>
                            </select>
                            @error('tipo_ferias')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" 
                                       name="data_inicio" 
                                       id="data_inicio" 
                                       value="{{ old('data_inicio', $lacationsLeave->data_inicio) }}"
                            ...
                            <div>
                                ...
                                <input type="date" 
                                       name="data_fim" 
                                       id="data_fim" 
                                       value="{{ old('data_fim', $lacationsLeave->data_fim) }}"
                        ...
                        <div>
                            ...
                            <textarea name="observacoes" 
                                      ...
                                      placeholder="Informações adicionais sobre as férias/licença...">{{ old('observacoes', $lacationsLeave->observacoes) }}</textarea>
                            @error('observacoes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
