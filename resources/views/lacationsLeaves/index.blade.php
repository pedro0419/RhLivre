            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-4 flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-gray-700">Lista de Férias e Licenças</h3>
                        <a href="{{ route('lacations-leaves.create') }}" 
                                    ...
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                {{ $lacationsLeave->tipo_ferias == 'Férias' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                                {{ $lacationsLeave->tipo_ferias }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ \Carbon\Carbon::parse($lacationsLeave->data_inicio)->format('d/m/Y') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ \Carbon\Carbon::parse($lacationsLeave->data_fim)->format('d/m/Y') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ \Carbon\Carbon::parse($lacationsLeave->data_inicio)->diffInDays(\Carbon\Carbon::parse($lacationsLeave->data_fim)) + 1 }} dias
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            <span class="line-clamp-2">{{ $lacationsLeave->observacoes ?: 'Sem observações' }}</span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('lacations-leaves.edit', $lacationsLeave->id) }}" 
                                                ...
                                                <form action="{{ route('lacations-leaves.delete', $lacationsLeave->id) }}" method="POST" class="inline"
